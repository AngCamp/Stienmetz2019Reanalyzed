...
##############################################################################
def calldata(recording, list_of_data, steinmetzpath = os.getcwd(),
    ...
    if propertysearch:
        for entry in list_of_data:
            if not(entry[-1:]=='.'):
                raise ValueError("propertysearch is true, all list_of_data entries must be properties which end in periods '.'\
    ...
    allsteinmetzrecordings = {'Cori_2016-12-14', 'Cori_2016-12-17', 'Cori_2016-12-18',
    ...
    def missing_recordings_warning(missing):
        ...
        warnings.warn('Not all recordings from Steimetz et al., 2019 are in the chosen directory.\
                      Please re-enter the recording value.\
                      The following recordings are missing: '+ missing)
    ...
    def recording_must_be_string():
        warnings.warn('recording variable must be a string.\
                      Please re-enter the recording value.')
        
    def incorrect_recording_entry(recording):
        warnings.warn(recording + ' is not a Steinmetz et al., 2019 session name.\
                      Please re-enter the recording value.')
    ...
    def incorrect_entry_warning(missing):
        ...
        missing = list(missing)
        missing =', '.join(missing)
        warnings.warn(missing + ' datatypes do not exist.\
    ...
    def regular_search(datalist):
        ...
        for entry in datalist:
            ...
            if entry[-3:]=='npy':
                ...
                out_dict[key[:-3]] = pd.read_csv(os.path.join(recording_path,entry),
                                                       sep = '\t',
                                                       header = 0)
        return(out_dict)
...
def recording_key():
    ...
    steinmetz_recordings_set = {'Cori_2016-12-14', 'Cori_2016-12-17', 'Cori_2016-12-18',
    'Forssmann_2017-11-01', 'Forssmann_2017-11-02', 'Forssmann_2017-11-04', 'Forssmann_2017-11-05',
    'Hench_2017-06-15', 'Hench_2017-06-16', 'Hench_2017-06-17', 'Hench_2017-06-18',
    'Lederberg_2017-12-06', 'Lederberg_2017-12-07', 'Lederberg_2017-12-08', 'Lederberg_2017-12-09',
    'Lederberg_2017-12-10', 'Lederberg_2017-12-11',
...
def datatype_key(supress_git_link = True, structured = False):
    ...
    """
    if not(structured):
        steinmetz_data_set = {'channels.brainLocation.tsv',
                   'channels.probe.npy',
                   'channels.rawRow.npy',
                   'channels.site.npy',
        ...
        steinmetz_data_set = {'channels.':['channels.brainLocation.tsv','channels.probe.npy', 'channels.rawRow.npy',
                   'channels.site.npy','channels.sitePositions.npy'],
                'clusters.':['clusters.depths.npy',
                   'clusters.originalIDs.npy','clusters.peakChannel.npy', 'clusters.probes.npy',
                   'clusters.templateWaveformChans.npy',
                   'clusters.templateWaveforms.npy',
                   'clusters.waveformDuration.npy',
                   'clusters._phy_annotation.npy'],
                   ...
                   'eye.timestamps.npy',
                   'eye.xyPos.npy'],
                   'face.':['face.motionEnergy.npy',
                   'face.timestamps.npy'],
                   'lickPiezo.':['lickPiezo.raw.npy',
                   'lickPiezo.timestamps.npy'],
                   'licks.':'licks.times.npy',
                   'passive':['passiveBeeps.times.npy',
                   'passiveValveClick.times.npy',
                   'passiveVisual.contrastLeft.npy',
                   'passiveVisual.contrastRight.npy',
                   'passiveVisual.times.npy',
                   'passiveWhiteNoise.times.npy'],
                   'probes.':['probes.description.tsv',
                   'probes.insertion.tsv',
                   'probes.rawFilename.tsv',
                   'probes.sitePositions.npy'],
                   'sparseNoise.':['sparseNoise.positions.npy',
                   'sparseNoise.times.npy',],
                   'spikes.':['spikes.amps.npy',
                   'spikes.clusters.npy',
                   'spikes.depths.npy',
                   'spikes.times.npy'],
                   'spontaneous.':'spontaneous.intervals.npy',
                   '.imec.lf.timestamps':['time stamps of probe recordings, first and last sample time, names and number of probes vary between recordings'],
                   'trials.':['trials.feedbackType.npy',
                   'trials.feedback_times.npy',
                   'trials.goCue_times.npy',
                   'trials.included.npy',
                   'trials.intervals.npy',
